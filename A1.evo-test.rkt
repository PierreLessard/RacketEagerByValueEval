...
; Test case 1: Update to environment with set!
(check-equal? (evo '((λ (x) (set! x 5)) 1)) 
              `(,Void #hash((E1 . ((x #&5) E0))
                            (λ0 . ((λ (x) (set! x 5)) E0)))))
; test the procedure which performs assignments namely set!

; Test case 2: add1
(check-equal? (evo `(,add1 1)) 
              '(2 #hash()))
; test the procedure to perform a racket function

; Test case 3: add1 and update with set!
(check-equal? (evo `((λ (x) (set! x (,add1 x))) 10))
                `(,Void #hash((E1 . ((x #&11) E0))
                              (λ0 . ((λ (x) (set! x (,add1 x))) E0)))))
; test the procedure can perform a racket function and update environment

; Test case 4: nested add1 and update with set!
(check-equal? (evo `((λ (x) (set! x (,add1 x))) (,add1 9)))
                `(,Void #hash((E1 . ((x #&11) E0))
                              (λ0 . ((λ (x) (set! x (,add1 x))) E0)))))
; test the procedure can perform a racket function and update environment 
; with a racket function

; Test case 5: Test evaluating a literal term
(check-equal? (evo 42) '(42 #hash()))
...
(check-equal? (evo 'x) '(x #hash()))
; This test case checks if the evaluator correctly evaluates a variable term to itself.

; Test case 7: Test evaluating a closure term
(check-equal? (evo '(λ (x) 1)) 
                  '(λ0 #hash(
                     (λ0 . ((λ (x) 1) E0)))))
; This test case checks if the evaluator correctly evaluates a closure term to itself.

; Test case 8: Testing boxing
(check-equal? (evo '((λ (x) x) 2)) 
                  '(2 #hash((E1 . ((x #&2) E0))
                     (λ0 . ((λ (x) x) E0)))))
; This test case checks if the evaluator correctly boxes
